...
- **`FhirSyntheticCollector`**: Gerador e parser de Bundles FHIR R4 sinteticos contendo recursos de `Patient`, `Encounter`, `Condition`, `Observation`, `Procedure` e `Organization` baseados no contexto demografico e epidemiologico brasileiro.

### 2.2. Validadores de Qualidade (`src/validators/`)
...
### 2.3. Seguranca e Conformidade LGPD (`src/lgpd/`)
- **`PIIDetector`**: Mapeamento declarativo via `config/pii_manifest.yaml` para deteccao automatica de colunas contendo dados pessoais (`NASC`, `N_AIH`, `CPF_AUT`, `CPF_PROF`, etc.).
...
### 2.4. Servico de Terminologias (`src/silver/terminology.py`)
...
### 2.5. Mapeadores Semanticos & Resolucao de Entidades (`src/silver/`)
- **Mapeadores Canocicos (`mappers/`)**: Conversores dedicados para transformar microdados do SIH e CNES em entidades dimensionais e fatos alinhadas ao padrao FHIR.
...
### 2.6. Persistencia Lakehouse (`src/lakehouse/`)
...
├── src/
│   ├── collectors/                # Modulos de extracao e descompressao
...
├── run_fhir_ingestion.py          # Script standalone para ingestao FHIR
├── run_silver_pipeline.py         # Script standalone para pipeline Silver
├── test_datasus_live_extraction.py# Script de teste de extracao real DATASUS
└── verify_silver.py               # Inspecao e validacao das tabelas Silver
```
...
A suite cobre testes unitarios de anonimizacao, validadores, gravacao Delta Lake, terminologias e resolucao de identidades:
...
- **Ingestao de dados clinicos FHIR (Bronze)**:
```bash
python run_fhir_ingestion.py
```

- **Extracao e ingestao real do DATASUS FTP (Bronze + Silver)**:
```bash
python test_datasus_live_extraction.py
```
